...
module.exports = {    //esportiamo tutto per JEST

  // Controlla che la word ci stia (in altezza) nella row
  fits: function(word, row){
  ...
  updateAverageHeight: function(row) {
    var sum = 0;
    var count = 0;
  ...
  analyzeReceipt: function (results) {
    ...
    for(var word of words){
      found = false;
      for(var row of rows){
        if(module.exports.fits(word, row)){
          found = true;
          row.words.push(word);
          module.exports.updateAverageHeight(row);
          break;
        }
      }
      if(!found){
    ...
    var mergedRows = [], i = 0, isFirstRow = true,
    lastRow = {
        words: [],
        isValid: false,
        price: null,
    ...
    for(var i = 1; i < mergedRows.length; i++){
      if(module.exports.isDiscount(mergedRows[i])) {
    ...
    return module.exports.printRow(final);
