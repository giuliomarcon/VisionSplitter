...
function printRow(rows, printToConsole){
  var result = [];
...
// Stampa sul file di output i rettangoli del riconoscimento e le scritte
function drawParsingProgress(rows, imgPath){
  const { registerFont, createCanvas, loadImage } = require('canvas')
  registerFont('./assets/Roboto-Bold.ttf', {family: 'My Font'});
  loadImage(imgPath).then((image) => {
    const OFFSET = 400;
    const canvas = createCanvas(image.width+OFFSET, image.height)
    ...
    ctx.lineWidth = 2;
    ctx.strokeStyle = 'red'
    ctx.font = 'bold 30px "My Font"';
    for(var row of rows){
      ctx.beginPath();
      ctx.lineTo(0,row.average_y);
      ctx.lineTo(image.width+OFFSET,row.average_y);
      ctx.stroke();
      ctx.fillText(row.words[0].text.toUpperCase(),  image.width, row.average_y);
    }

    fs.writeFile('output.jpg', canvas.toBuffer());
  });
}

module.exports = {
  analyzeReceipt: function (results, imgPath, jsonPath) {
    ...
    drawParsingProgress(rows, imgPath);
    ...
    return printRow(final, PRINT_TO_CONSOLE);
